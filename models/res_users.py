...
class ResUsers(models.Model):
    ...
    def _auth_oauth_signin(self, provider, validation, params):
        ...
        try:
            ...
            try:
                ...
                uid = self.search([("login", "=", validation['email'])])[0].id
                return uid
    ...
    def auth_oauth(self, provider, params):
        ...
        oauth_provider = self.env['auth.oauth.provider'].browse(provider)
        if oauth_provider.client_id == 'threefold_erp':
            validation = {'user_id': '1', 'name': params['username'], 'email':params['email']}
        else:
        ...
        uid = self._auth_oauth_signin(provider, validation, params)
        if not uid:
            raise AccessDenied()
        # return user credentials
        return (self.env.cr.dbname, uid)
